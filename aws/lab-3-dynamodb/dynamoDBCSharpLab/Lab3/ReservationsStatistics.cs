...
namespace Lab3
{
  ...
  public static class ReservationsStatistics
  {
    ...
    private static Dictionary<string, AttributeValue> MakeAttrValues(string key, string value) => new Dictionary<string, AttributeValue> { { key, new AttributeValue { S = value } } };

    private static QueryResponse QueryCityRelatedItems(string inputCity) => dynamoDBClient.Query(new QueryRequest
    {
      TableName = ReservationsTableName,
      IndexName = CityDateIndexName,
      KeyConditionExpression = "City = :v_city",
      ExpressionAttributeValues = MakeAttrValues(":v_city", inputCity)
    });
  }
}
